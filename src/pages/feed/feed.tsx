...
export const Feed = () => {
  ...
  const doneOrders = useMemo(
    () => orders && orders.filter((order: TOrderType) => order.status === "done"),
    [orders]
  );
  const pendingOrders = useMemo(
    () => orders && orders.filter((order: TOrderType) => order.status === "pending"),
    [orders]
  );
              ...
              <div className={styles.feed__ordersStatus}>
                <div>
                  ...
                  <ul className={styles.feed__ordersList}>
                    {doneOrders &&
                      doneOrders.slice(0, 10).map((order: TOrderType) => (
                        <li key={order._id}>
                          <p
                ...
                <div>
                  ...
                  <ul className={styles.feed__ordersList}>
                    {pendingOrders &&
                      pendingOrders.map((order: TOrderType) => (
                        <li key={order._id}>
