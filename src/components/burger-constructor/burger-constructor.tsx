...
const BurgerConstructor = () => {
  ...
  const [, drop] = useDrop(() => ({
    ...
    drop: (item: TIngredientType) => dispatch(addToConstructor(item)),
    collect: (monitor) => ({
    ...
    if (isAuth) {
      ...
      dispatch(
        orderBurger([
          ...
          ...constructorItems.ingredients.map((el: TIngredientType) => el._id),
          constructorItems.bun._id,
        ])
      );
    } else {
  ...
  const price = useMemo(() => {
    return (
      ...
      constructorItems.ingredients.reduce((sum: number, el: TIngredientType) => sum + el.price, 0)
    );
  }, [constructorItems]);
          ...
          <div className="m-4">
            ...
            <ul className={style.elements}>
              {constructorItems.ingredients.length > 0 ? (
                constructorItems.ingredients.map((item: TIngredientType, index: number) => {
                  return (
